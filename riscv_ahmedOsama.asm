...
newline:
	.asciz "\n"
message:
	.asciz "please enter the size of the board"
message_tree:
	.asciz "do you want to enter a tree? if yes press 1 if no press 0"
message_tree_coordinates:
	.asciz " please enter the coordinates of your tree the row first and then the column"
	
solution_not:
	 .asciz "No solution has been found"
...
li a7, 5                      		 # enter your size
ecall
add t0, a0, zero			 # store the size of the array
li s10, 1
pow:
	li s11, 2
	mul s10, s10, s11
...
minus1:
	li t2, 0
	li s11, 1			 # set t2 to zero because we will use it again
	sub a4, s10, s11		 # this a4 will always be constant that we use in bitwise operators to take only
...
# at the start of this function we have the size at t0 then the function will print the board GIVEN the board is square of course
jal print_board
...
li a7,5				 # enter whether you want to enter trees or not				
ecall
li s11, 1
beq a0, s11, enter_array			 # if he wants to enter trees at all go in
beq a0, zero, exit_enter_tree 	 # if he doesn't want to add trees at all exit
bne a0, zero, enter_trees		 # if it is none of the two values he has to enter one of them
...
li a7, 5 				 # enter row coordinate
ecall
add t1, a0, zero			 # the row coordinate is now in t1

li a7, 5 				 # enter column coordinate
ecall
add t2, a0, zero			 # the y column is now in t1

# now we calulate the place of the tree inside the 2-D array
mul s9, t1, t0 			 # multiply size by row to get at the beginnig of row you want
add s9, s9, t2
li s11, 4			 # add the number of columns to get at the element you want in that row
mul s9, s9, s11				 # multiply by 4 since every int is stored in 4 bytes 

add s9, t4, s9			 # add the number of bytes to the reference address to get the address value to store in in s9

li a7, 2

sw a7, (s9)                            # store the integer in memory

again_choose_tree:

li a7, 4 # print a message asking if there is more
la a0, message_tree 
ecall
...
ecall
li s11, 1
beq a0, s11, enter_array			 # if he wants to enter another tree
beq a0, zero, exit_enter_tree 	 # if he doesn't want to add anymore trees
bne a0, zero, again_choose_tree	 # if it is none of the two values he has to enter one of them
...
la a0, newline
li a7, 4				 # print a newline after every row
ecall
...
lizards:				 # t0 is used to get size of the array
					 ...
					 # t6 is used for rd
					 # s8 is used for col
					 # s9 is used for getting suitable amount of bits
addi sp, sp, -4			         # get more stack room
sw  ra, (sp) 				 # store ra for the first main function or in other words  for lizard functions
beq a7, t0, enditall			 # if the number of lizards equal the size of the board then you killed it 
beq t2, t0, no_solution			 # the is the condition that we can't find a solution with our method
li a7, 0				 # set the number of lizards to zero at each trial
...
li s8, 0				 # for each trial the col is initialized
...
addi t2, t2, 1				 # in case one of the cells in the first row fails we need to start the second time with column next
addi t4, t4, 4				 # if one of the cells in the first row didn't succeed we start all over by the next one
...
lw s10, (t4)				 # we get the value stored in the board to know if it is a tree or not

li s11, 2
beq s10, s11, recurse_tree		 # checking if it is a tree we will recurse on different conditions	 
...
li s11, 1
beq a0, s11, not_safe           		 # if the row has a lizard that is not masked


or a3, t5, t6			 # the values of rd and ld for this row are considered
or a3, a3, s8			 # also the lizards in columns are considered


li s11, 1
sub a1, t0, s11			         # get the maximum allowed column index in a1
sub a1, a1, t2			 # subtract the max value from the actual value of the column, to get in reverse
li a2, 1
sll a2, a2, a1			 # we are going to shift logical by a1 and the value of a2 will be the bin of current column
...
addi a7, a7, 1				 # this will act as a counter for the number of lizards we have 
...
or s8, a2, s8			 # we are going to or the current column and all the previous column to get a binary for all 
...
recurse_next_row:
srli t5, t5, 1				 # to update the effect of a diagonal on the next row, you must shift it to right by one
and t5, t5, a4			 # this will ensure the data has info only for places inculded in the board
slli t6, t6, 1				 # to update the effect of a right diagonal on the next row, shift it to left by one
and t6, t6, a4			 # this will ensure the data has info only for places inculded in the board
...
recurse_tree:
li s11, 1					 # before we do any checking we need to apply the masking of the tree
sub a1, t0, s11			         # get the maximum allowed column index in a1
sub a1, a1, t2			 # subtract the max value from the actual value of the column, to get in reverse
li a2, 1
sll a2, a2, a1			 # we are going to shift logical by a1 and the value of a2 will be the bin of current column
...
and s8, s8, a2			 # mask the effect of the column by the tree

					 # we first check below the tree if we can place a lizard we do and continue horizontal 
					 # movement from there
li s11, 4
mul t0, t0, s11	         		 # get the number of bytes required to get to the next row
add t4, t4, t0			 # add to the current index the size of the 2-D array in bytes which will get it to the next element in column	
li s11, 4
div t0, t0, s11  			 # get the size back without the bytes 
lw s10, (t4)				 # get the value in that index in case we have another foken tree	 
addi t1, t1, 1				 # since we will be checking below we increment the row because we will work from that row from now on
...
li s11, 2
beq s10, s11, check_next_cell		 # if the cell_below tree is another tree, check the cell next to the upper tree
					  

or a3, t5, t6			 # only the values of the diagonals may affect since the tree already masked col
or a3, a3, s8			 # we also need to update col num
and a2, a3, a2
...
li s11, 1					 # # in case all conditions are met we do indeed put 1 under the tree
beq a0, s11, check_next_cell

li a0, 1				 # this will  also be used as an indication that there is a  lizard in the column
sw a0, (t4)				 # put 1 in that empty_safe cell and proceed 
addi a7, a7, 1				 # this will act as a counter for the number of lizards we have 
addi s5, s5, 1          		 # since we jump under the tree put one and return one row the effect of the one
...
li s11, 4					 # of course we will keep the masking since the tree is not going anywhere 
mul t0, t0, s11			         # get the number of bytes required to get to the next row
sub t4, t4, t0
li s11, 4
div t0, t0, s11  			 # get the size back withouth bytes
li s11, 1
sub t1, t1, s11
slli t5, t5, 1				 # since we got back one row we restore the ld of that row
or t5, t5, t3			 # restore truncated errors due to double jump and get back
srli t6, t6, 1				 # since we got back one row we restore the rd of that row
li s11, 1
sub a1, t0, s11				 # get the maximum allowed column index in a1
beq t2, a1, recurse_next_row 		 # we need this to be in sparate label because we we go to next row we need to update data_structure
...
slli t5, t5, 1				 # since we got back one row we restore the ld of that row
or t5, t5, t3			 # restore truncated errors due to double jump and get back
srli t6, t6, 1				 # since we got bacl one row we restore the rd of that row
li a0, 0				 # since this is the row that contains the tree it masks any affect if any of a previous

li s11,1					 # lizard in the row
sub a1, t0, s11				 # get the maximum allowed column index in a1
beq t2, a1, recurse_next_row 		 # we need this to be in sparate label because we we go to next row we need to update data_structure
...
jr ra
no_solution:
la a0, solution_not        			 # load the message about the size
li a7, 4
ecall
